...
namespace ShiftManagement.Services
{
    public class ExportService
    {
        ...
        public ExportService(ShiftManagementContext context)
        {
            _context = context;
        }

        public async Task<(List<AttendanceExportDto> Data, string? Error)> GetAttendanceExportDataAsync(int departmentId, string period)
        {
            if (!DateTime.TryParse(period + "-01", out DateTime monthStart))
                return (null, "Invalid period format. Use yyyy-MM");
        ...
        public byte[] ExportAttendanceToExcel(List<AttendanceExportDto> dtoData)
        {
        ...
        public byte[] ExportAttendanceToPdf(List<AttendanceExportDto> dtoData)
        {
